...
class TestDesignWizard(unittest.TestCase):
    ...
    def test_default_values(self):
        ...
        self.assertTrue(set(["class", "function", "import", "call",
                             "expression"]).
    ...
    def test_values_of_inner_functions(self):
        ...
        self.assertEqual(class_entity1.get_functions_str(), ['inside_func'])
    ...
    def test_body_not_empty_function(self):
        ...
        self.assertNotEqual(func_entity1.get_function_fields_body(), [])
    ...
    def test_get_calls_inside_functions_body(self):
        ...
        self.assertEqual(
            func_entity.get_function_calls_str(),
            [('caller', ['func2']), ('callee', [])]
        )
        self.assertEqual(
            func_entity.get_function_calls_str(just_caller=True), ['func2'])
        self.assertEqual(
            func_entity.get_function_calls_str(just_callee=True), [])
    ...
    def test_updating_function_calls(self):
        ...
        self.assertEquals(
            i.get_function_calls_str(just_caller=True),
            ['print', 'map', 'sort']
        )
    ...
    def test_get_expression_nodes(self):
        ...
        self.assertEquals(
            list(
             self.dw.entities.keys()),
            [
             'sort', 'func2', 'for', 'map',
             'sum', 'while', 'range', 'split',
    ...
    def test_nested_for(self):
        ...
        self.dw.design_populate_loop_entities('for')
        self.assertTrue(
    ...
    def test_nested_while(self):
        self.dw.design_populate_all_entities()
        self.dw.design_populate_loop_entities('while')
        self.assertTrue(
            self.dw.design_get_relations_from_entity(
                'while1', 'HASLOOP') != [])
        node_callee = \
...
if __name__ == '__main__':
    suite = unittest.TestLoader().\
     loadTestsFromTestCase(TestDesignWizard)
